...
_REASON_MESSAGES: dict[ReasonCode, str] = {
    ReasonCode.CCCD_IMAGE_QUALITY_LOW: (
        "The CCCD image is too blurry, too dark, or has too much glare."
    ),
    ReasonCode.CCCD_PORTRAIT_NOT_FOUND: (
        "The portrait could not be extracted from the CCCD image."
    ),
    ...
    ReasonCode.LIVE_VIDEO_QUALITY_LOW: (
        "The live video is too short or does not meet quality requirements."
    ),
    ReasonCode.FACE_OCCLUDED: "The face is partially covered and cannot be verified reliably.",
    ReasonCode.LIVENESS_CHALLENGE_FAILED: (
        "The live liveness challenge was not completed successfully."
    ),
...
def reason_message(reason_code: ReasonCode | None) -> str | None:
    ...
    return _REASON_MESSAGES.get(
        reason_code,
        "The verification attempt could not be completed.",
    )
