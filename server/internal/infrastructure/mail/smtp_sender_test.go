...
func smtpPeer(t *testing.T, serve func(net.Conn) error) *domain.SMTPConfig {
 ...
 go func() {
  ...
  defer conn.Close()
  if err := conn.SetDeadline(time.Now().Add(3*time.Second)); err != nil { done <- err; return }
  done <- serve(conn)
 }()
 t.Cleanup(func() {
  _ = listener.Close()
  if err := <-done; err != nil { t.Errorf("SMTP peer: %v", err) }
 })
...
func smtpExchange(reader *bufio.Reader, conn net.Conn, prefix, reply string) error {
 ...
 if !strings.HasPrefix(line, prefix) { return fmt.Errorf("command %q, want prefix %q", line, prefix) }
...
func TestSMTPSenderSend(t *testing.T) {
 ...
 cfg := smtpPeer(t, func(conn net.Conn) error {
  ...
  for _, step := range []struct{ command, reply string }{
   ...
   if err := smtpExchange(reader, conn, step.command, step.reply); err != nil { return err }
  }
 ...
 case message := <-body:
  for _, want := range []string{"Subject: OTP", "123456", "<p>123456</p>", "multipart/alternative"} {
   if !strings.Contains(message, want) { t.Errorf("message missing %q", want) }
  }
 default: t.Fatal("SMTP peer received no message")
 }
}
...
func TestSMTPSenderRecipientRejected(t *testing.T) {
 cfg := smtpPeer(t, func(conn net.Conn) error {
  ...
  line, err := reader.ReadString('\n')
  if err != io.EOF || line != "" { return fmt.Errorf("after rejection: command %q, error %v", line, err) }
  return nil
 })
 sender, err := NewSMTPSender(cfg)
 if err != nil { t.Fatal(err) }
...
func TestSMTPSenderDeadlineBoundsStalledGreeting(t *testing.T) {
 ...
 if !errors.As(err, &netErr) || !netErr.Timeout() { t.Fatalf("Send() error = %v, want timeout", err) }
 if time.Since(started) > time.Second { t.Fatal("request deadline did not bound SMTP operation") }
 select { case <-connected: default: t.Fatal("did not connect to stalled peer") }
}
...
func TestSMTPSenderCanceledBeforeDial(t *testing.T) {
 ...
 cancel()
 if err := sender.Send(ctx, maildomain.MailMessage{}); !errors.Is(err, context.Canceled) { t.Fatalf("Send() error = %v", err) }
}
