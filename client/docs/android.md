...
Dùng **một emulator API 33: `pixel_33`** (Google APIs, x86_64).
Shell có image API 33; SDK 36 vẫn cần để biên dịch thư viện Tauri.
...
Nếu danh sách đã có `pixel_33`, bỏ qua bước tạo. Nếu chưa có, chạy:
...
Chỉ tạo một lần; lệnh không ghi đè `pixel_33` đã tồn tại.

Nếu đã vào shell Android và muốn chạy trực tiếp, lệnh tương đương là:

```bash
avdmanager create avd --name pixel_33 --package 'system-images;android-33;google_apis;x86_64'
```
...
Lệnh tự vào Nix shell Android. AVD `pixel_33` cần được tạo ở mục 1.
...
| Lỗi                            | Cách xử lý                                                                             |
| ------------------------------ | -------------------------------------------------------------------------------------- |
| `Unknown AVD name`             | Chạy `emulator -list-avds`; nếu chưa có `pixel_33`, tạo theo mục 1.                    |
...
```bash
ls "$ANDROID_HOME/system-images/android-33/google_apis/x86_64/system.img"
```
